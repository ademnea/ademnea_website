...
def generate_pdf(results, plots, start_year, start_month, end_year, end_month, hive_id, report_folder):
    """Generate a single PDF report with structured tables and plots for all months in the specified range."""
    start_month_name = date(start_year, start_month, 1).strftime('%B')
    end_month_name = date(end_year, end_month, 1).strftime('%B')
    filename = os.path.join(report_folder, f"Range_Report_{hive_id}_{start_year}_{start_month_name}_to_{end_year}_{end_month_name}.pdf")
    doc = SimpleDocTemplate(filename, pagesize=letter)
    styles = getSampleStyleSheet()
    story = []

    # Add title
    title = f"Range Analysis Report for {hive_id}: {start_month_name} {start_year} to {end_month_name} {end_year}"
    ...
    for month_key, month_results in results.items():
        ...
        year = int(year)
        story.append(Paragraph(f"Analysis for {month_name} {year}", styles['Heading1']))
        story.append(Spacer(1, 24))

        # Process each section
        for section, content in month_results.items():
            ...
            if isinstance(content, dict):
                ...
                # Handle CO2 Analysis
                if section == "CO2 Analysis":
                    # Statistics Table
                    stats = content.get('Statistics', {})
                    stats_data = [[key.replace('CO2', 'CO2'), format_value(value)] for key, value in stats.items()]
                    ...
                    diurnal_data = [[key.replace('CO2', 'CO2'), format_value(value)] for key, value in diurnal.items()]
                    ...
                    anomalies_data = [[format_value(item['record'])] for item in anomalies]
                    if anomalies_data:
                        anomalies_table = create_table(anomalies_data, headers=["Anomalous CO2 (ppm)"])
                        story.append(Paragraph("Anomalies", styles['Heading3']))
                        story.append(anomalies_table)
                        story.append(Spacer(1, 12))

                # Handle Weight Analysis
                elif section == "Weight Analysis":
                    # Statistics Table
                    stats = content.get('Statistics', {})
                ...
                # Handle Temperature Analysis
                    ...
                    # Correlation Table
                    corr = content.get('Correlation', {})
                    corr_data = [[key, format_value(value)] for key, value in corr.items()]
                    corr_table = create_table(corr_data, headers=["Metric", "Value"])
                    story.append(Paragraph("Correlation", styles['Heading3']))
                    story.append(corr_table)
                    story.append(Spacer(1, 12))

                # Handle Humidity Analysis
                elif section == "Humidity Analysis":
                    ...
                    story.append(Paragraph("Humidity Insights", styles['Heading3']))
                    story.append(humidity_table)
                    story.append(Spacer(1, 12))

                    # Correlation Analysis Table
                    corr_analysis = content.get('Correlation Analysis', {})
                    corr_data = [[key, format_value(value)] for key, value in corr_analysis.items()]
                    corr_table = create_table(corr_data, headers=["Metric", "Correlation Coefficient"], col_widths=[300, 200])
                ...
                # Handle Correlation Analysis
                    ...
                    corr_data = [[key.replace('CO2', 'CO2'), value] for key, value in content.items()]
                    corr_table = create_table(corr_data, headers=["Metric", "Correlation Coefficients"], col_widths=[200, 300])
                    story.append(corr_table)
                    story.append(Spacer(1, 12))
