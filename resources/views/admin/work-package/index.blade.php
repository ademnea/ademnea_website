               ...
               {{ $loop->iteration }}
                ...
                <td class="px-6 py-4">
                    <div class="flex space-x-2">
                        <a href="#" type="button" data-modal-target="{{ $item->name }}" data-modal-show="{{ $item->name }}" style="color: white; background-color: #28a745; padding: 6px 12px; border-radius: 5px; font-size: 0.75rem; font-weight: 600;" class="inline-flex items-center">View</a>
                        <a href="{{ url('/admin/work-package/' . $item->id . '/edit') }}" style="color: white; background-color: #ffc107; padding: 6px 12px; border-radius: 5px; font-size: 0.75rem; font-weight: 600;" class="inline-flex items-center">Edit</a>
                        <form action="{{ url('admin/work-package/' . $item->id) }}" method="POST" style="display:inline;">
                            @csrf
                            @method('DELETE')
                            <button type="submit" onclick="return confirm('Are you sure you want to delete {{ $item->name }}?')" style="color: white; background-color: #dc3545; padding: 6px 12px; border-radius: 5px; font-size: 0.75rem; font-weight: 600; border: none;" class="inline-flex items-center">Delete</button>
                        </form>
                    </div>
                ...
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <div class="flex items-center p-6 space-x-2 border-t border-gray-200 rounded-b dark:border-gray-600">
                     <div class="flex justify-center">
                         <button type="submit" id="back-button" onclick="window.location.href = '/admin/work-package'" data-modal-hide="{{ $item->name }}" class="text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:outline-none focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-green-600 dark:hover:bg-greeb-700 dark:focus:ring-green-800">
                             Back
                         </button>
                     </div>
                 </div>
...
<div class="relative w-full max-w-md max-h-full">
    <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
        ...
        <div class="p-6 text-center">
           ...
           <form id="delete-user-form-{{ $item->id}}" method="POST" action="{{ url('admin/work-package' . '/' . $item->id) }}" accept-charset="UTF-8" style="display:inline">
            {{ method_field('DELETE') }}
            {{ csrf_field() }}
 ...
 <script>
  ...
  $(".back-button").on("click", function() {
         ...
         window.location.href = "{{ url('admin/work-package') }}";
     });
 
    // Event listener for back button click
 document.getElementById("back-button").addEventListener("click", function() {
    // Redirect to the team page
    window.location.href = "{{ url('admin/work-package') }}"; // Replace with the actual URL of the team page
 });
   ...
   $('#addTeamForm').on('submit', function(event) {
         ...
         $.ajax({
             ...
             success: function(response) {
                 ...
                 // Redirect to the team page
                 window.location.href = "/admin/work-package";
             }
         });
     });
 </script>
